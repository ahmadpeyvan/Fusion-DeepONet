...
# def apply_dropout(x, p, key):
#     return jnp.where(random.bernoulli(key, p, x.shape), x / p, 0)
...
def plot_basis(xx, yy, u_pred, title_prefix, file, vmin, vmax):
    ...
    cbar1 = fig.colorbar(tpc1, ax=ax, orientation='vertical', fraction=0.046, pad=0.04)
    cbar1.set_label('Color Scale', fontsize=30)  # label font size
...
def apply_dropout_last_axis(x, p, key):
    # Generate a mask with shape matching x along the last axis only
    mask_shape = list(x.shape)
    # print("x=\t",x.shape)
    # print("mask_shape=\t",mask_shape)
    mask_shape[-1] = 1  # Set last axis to 1 for broadcasting
    # print("mask_shape1=\t",mask_shape)
    mask = random.bernoulli(key, p, mask_shape) / p  # Create mask with keep rate p
    # print("mask=\t",mask)
    return x * mask  # Broadcast the mask along the last axis
...
plot_dir = "./plots_basis/"

path="./models/"

model = "model.4"

os.system("mkdir plots_basis")
...
def predict(params, data):
    ...
    u_out_trunk,u_out_branch,trunk_out_l = fnn_fuse_oneway(x,v,[W_trunk, b_trunk,a_trunk, c_trunk,a1_trunk, F1_trunk, c1_trunk]
...
filename = path+model
params = load_model(filename)
u_pred,trunk_out_l = predict(params, [v_train, x_train])
u_pred_test,trunk_out_l_test = predict(params, [v_test, x_test])

print("trunk_out_l=\t",len(trunk_out_l))

# for l in range(len(trunk_out_l)):
#     print("layer_output=\t",trunk_out_l[l].shape)
#     
...
for i in range(len(trunk_out_l)):
    ...
    for j in range(G_dim):
        xx, yy = x_train[0, :,0].reshape(256,256,order='C'), x_train[0, :,1].reshape(256,256,order='C')
        vmin, vmax = U[:,j].min(axis=(0)), U[:,j].max(axis=(0))
        ...
        # print(vertices)
        basis = U[:,j].reshape(256,256,order='C')
        file = os.path.join(plot_dir, f"basis_{i}_{j}.png")
        plot_basis(xx, yy, basis, "Basis", file, vmin, vmax)


# plt.figure(figsize=(10, 6))
# for i in range(len(eigens)):
#     plt.plot(x, eigens[i], label=f'Layer {i+1}')
# plt.xlabel('Modes',fontsize=16)
# plt.ylabel('Spectrum',fontsize=16)
# plt.yscale('log')
# plt.ylim([1e-9,10.0])
# plt.title('Fusion DeepONet Spectrum')

# plt.xticks(fontsize=18)
# plt.yticks(fontsize=18)

# plt.legend(loc='best', bbox_to_anchor=(0.9, 1), fontsize=16, ncol=1)
# plt.tight_layout()
# filename = os.path.join(plot_dir, f"spectrum.png")
...
# # plt.yscale('log')
# # plt.title('Shits')
# # plt.legend(loc='upper right', bbox_to_anchor=(1.2, 1), fontsize='small', ncol=2)
# # plt.tight_layout()
# # plt.show()
...
# # np.savez("l2_test_FusionDeepONet_RG.npz")
