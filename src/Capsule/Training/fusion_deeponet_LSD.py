...
filename_loss = './models_derivative/loss'

os.system('rm -r models_derivative')
os.system('mkdir models_derivative')
...
def save_model(param,n):
    filename = './models_derivative/model'+'.'+str(n)
...
def loss(params, data, u,u_der,der_lim):
    ...
    # u_preds_der = jnp.log(u_preds_der-dermin*jnp.ones_like(u_preds_der)+jnp.ones_like(u_preds_der))
...
def loss_for_test(params, data, u,u_der,der_lim,nn_idx_batch):
    u_preds = predict(params, data)
    v, x = data
    ...
    # mind = jnp.min(u_preds_der, axis=(0,1), keepdims=True)
    # maxd = jnp.max(u_preds_der, axis=(0,1), keepdims=True)

    # u_preds_der = (u_preds_der - dermin)/(dermax - dermin)
    # u_preds_der = jnp.log(u_preds_der-mind*jnp.ones_like(u_preds_der)+jnp.ones_like(u_preds_der))
...
# scale every thin to 1<x<max
# u_tr_der = jnp.log(u_tr_der-dermin*jnp.ones_like(u_tr_der)+jnp.ones_like(u_tr_der))
# u_te_der = jnp.log(u_te_der-dermin*jnp.ones_like(u_te_der)+jnp.ones_like(u_te_der))
...
for epoch in range(num_epochs_adam):
    ...
    if epoch % 10 ==0:
        ...
        loss_test,l2_test,l2_diff_test,ll_test,ll1_test= loss_for_test(params, [v_te, x_te],u_te,u_te_der,der_lim,nn_idx_batch_jax_te)
        losss,l2_train,l2_diff_train,ll_train,ll1_train= loss_for_test(params, [v_tr, x_tr],u_tr,u_tr_der,der_lim,nn_idx_batch_jax)
        
        train_loss.append(losss)
        test_loss.append(loss_test)
        epo.append(epoch)
        print("Epoch {} | T: {:0.6f} | Train MSE: {:0.3e} | Test MSE: {:0.3e} | MSE: {:0.3e}| Lossder: {:0.3e} | Train L2: {:0.5f} | Test L2: {:0.5f} | Diff_Train L2: {:0.5f} | Diff_Test L2: {:0.5f}".format(epoch, epoch_time,losss,loss_test,ll_train,ll1_train,l2_train, l2_test,l2_diff_train,l2_diff_test))
