...
filename_loss = './models_autodiff/loss'

os.system('rm -r models_autodiff')
os.system('mkdir models_autodiff')
...
def save_model(param,n):
    filename = './models_autodiff/model'+'.'+str(n)
...
def loss(params, data, u,u_der,der_lim):

    v, x = data
    u_preds = predict_sample_vect (params, x,v)
    u_preds_der = jac_fn_rev_sample_vect(params, x,v)
...
def loss_for_test(params, data, u,u_der,der_lim):
    v, x = data
    u_preds = predict_sample_vect(params, x,v)
    u_preds_der = jac_fn_rev_sample_vect(params, x,v)
    ...
    l2_der = jnp.mean(jnp.linalg.norm(u_der.flatten() - u_preds_der.flatten(), 2)/jnp.linalg.norm(u_der.flatten() , 2))

    return mse,l2,l2_der,loss,loss1

@jit
def update(params, data, u,u_der,der_lim, opt_state):
    """ Compute the gradient for a batch and update the parameters """
    value, grads = value_and_grad(loss,argnums=0,has_aux=False)(params, data, u,u_der,der_lim)
...
# vectorizing the predict function
...
# pred_ders=jac_fn_rev_sample_vect(params,x,v)
# print("pred_ders=\t",pred_ders.shape)
# preds = predict_sample_vect(params,x,v)
...
nn_idx_batch = np.stack([compute_nn_idx(x_tr[b], k=40) for b in range(x_tr.shape[0])], axis=0)
nn_idx_batch_jax = jnp.array(nn_idx_batch)

nn_idx_batch = np.stack([compute_nn_idx(x_te[b], k=40) for b in range(x_te.shape[0])], axis=0)
nn_idx_batch_jax_te = jnp.array(nn_idx_batch)

# (coords, values, v, nn_idx, k=5)
batched_function_var = vmap(directional_derivative_single_case_jax, in_axes=(None, -1, None, None),out_axes=-1)

batched_function = vmap(batched_function_var, in_axes=(0, 0, None, 0))
batched_function_test = vmap(batched_function_var, in_axes=(0, 0, None, 0))
...
# print("u_tr_der_x=\t",u_tr_der_x.shape)
u_tr_der = jnp.concatenate((u_tr_der_x[:,:,:,None],u_tr_der_y[:,:,:,None]),axis=-1)
u_te_der = jnp.concatenate((u_te_der_x[:,:,:,None],u_te_der_y[:,:,:,None]),axis=-1)
...
loss_train= loss(params, [v_tr, x_tr],u_tr,u_tr_der,der_lim)
...
for epoch in range(num_epochs_adam):
    ...
    params, opt_state, loss_val = update(params, [v_tr,x_tr], u_tr,u_tr_der,der_lim, opt_state)
    losss = loss_val
    # print(epoch,losss)
    if epoch % 10 ==0:
        epoch_time = time.time() - start_time
        loss_te,l2_test,l2_diff_test,mse_test,derloss_test= loss_for_test(params, [v_te, x_te],u_te,u_te_der,der_lim)
        loss_tr,l2_train,l2_diff_train,mse_train,derloss_train= loss_for_test(params, [v_tr, x_tr],u_tr,u_tr_der,der_lim)
        train_loss.append(loss_tr)
        test_loss.append(loss_te)
        epo.append(epoch)
        print("Epoch {} | T: {:0.6f} | Train MSE: {:0.3e}| MSE: {:0.3e}| Der MSE: {:0.3e} | Test MSE: {:0.3e} | Train L2: {:0.5f} | Test L2: {:0.5f} | Diff_Train L2: {:0.5f} | Diff_Test L2: {:0.5f}".format(epoch, epoch_time,loss_tr,mse_train,derloss_train,loss_te,l2_train, l2_test,l2_diff_train,l2_diff_test))
